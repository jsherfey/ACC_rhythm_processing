...
switch cfg.INPUTTYPE
  ...
  case 2 % sinusoidal input [0,1]
    ...
    if numel(cfg.F0)==1
      ...
      %       I=I-min(I(:));
      %       I=I/max(I(:));
      %     else % deliver each sin(f0(k)) to a distinct subset of cells
      %       nf0=length(f0);
      %       grpsize=ceil(Npop/nf0);
      %       targets=1:grpsize;
      %       I=zeros(Npop,nt);
      %       for k=1:nf0
      %         i=sin(2*pi*f0(k)*t/1000);
      %         i=i-min(i(:));
      %         i=repmat(i,[grpsize 1]);
      %         I(targets,:)=i;
      %         targets=targets+grpsize;
      %         targets=targets(targets<=Npop);
      %         if isempty(targets)
      %           %targets=1:Npop;
      %         end
      %         grpsize=length(targets);
      %       end
      %     end
    end
  ...
  case 6 % Poisson-exponential (poissrnd method)
    ...
    I(I<0)=0;
%     max(I(:))
    %denom=repmat(max(I(:,on:off),[],2),[1 nt]);
    denom=repmat(prctile(I(:,on:off),95,2),[1 nt]);
    %denom=repmat(mean(I(:,on:off),2),[1 nt]);
