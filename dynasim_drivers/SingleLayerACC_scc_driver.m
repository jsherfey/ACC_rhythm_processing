...
%{
Potential relationship b/w my ACd classes and Fiona's manual Groups:
Group 1 <-> Class 3
...
rootoutdir=fullfile(project_dir,'dynasim_studies');
model_path=fullfile(project_dir,'model_files');
addpath(fullfile(project_dir,'analysis_functions'));
addpath(fullfile(project_dir,'dynasim_experiments'));
addpath(model_path);
cd(model_path);
...
E_type='ACd_Class1'; % options: {'PY','ACd_Class1','ACd_Class2','ACd_Class3','ACd_Class4'; todo: 'ACC_sup','ACC_deep'} (differentiated by rat ACd PY IPs measured by Natalie Adams)
  ...
  % TODO: multi-class layers
  %   'ACd_sup124': 63% Class1, 21% Class2, 16% Class4
  %   'ACd_deep124': 40% Class1, %50% Class2, 10% Class4
  %   'ACd_sup12': 75% Class1, 25% Class2
  %   'ACd_deep12': 45% Class1, 55% class2
  %   'ACd_sup_min': 100% Class1
  %   'ACd_deep_min': 50% Class1, 50% Class2
  %   'ACd_deep_groups': 33% Class1, 33% Class2, 33% Class4
  %   (layers including Class3)
If_type='HH';     % options: {'HH'; todo: 'FS','RSNP','LTS'}    (differentiated by electrophysiological class)
Is_type='HH';     % options: {'HH'; todo: 'FS','RSNP','LTS'}    (differentiated by electrophysiological class)
% select heterogeneity degree and E-cell model parameters to distribute
heterogeneity_degree=1; % options: scalar scaling factor
distribution='normal';  % options: {'uniform','normal','lognormal'}
heterogeneous_params={}; % options: {'Eleak','gh'}
% -------------------------------------------------------------------------
...
onset=50;   % ms, start time of injected current
offset=inf; % ms, stop time of injected current

% Poisson noise parameters
% baseline_rate_E=3.5;%.1; % kHz, baseline poisson rate to E-cells
% baseline_rate_E=10;%.1; % kHz, baseline poisson rate to E-cells
switch E_type 
  % todo: move this switch statement to ACC_Ecell_specification.m
  case 'ACd_Class1'
    baseline_rate_E=.5;%.1; % kHz, baseline poisson rate to E-cells
  case 'ACd_Class2'
    baseline_rate_E=3.5;%.1; % kHz, baseline poisson rate to E-cells
  case 'ACd_Class3'
    baseline_rate_E=.25;%.1; % kHz, baseline poisson rate to E-cells
  case 'ACd_Class4'
    baseline_rate_E=3.5;%.1; % kHz, baseline poisson rate to E-cells
end
...
% note: noise parameters chosen s.t. baseline cell FR ~ 1-4Hz in {iNa,iK} cell
...
noise_parameters={'kick',kick,'gNOISE',gNOISE,'tauAMPA',tauAMPA,'EAMPA',EAMPA,'onset',onset,'offset',offset};
fixed_biophysics={'v_IC',v_IC,'v_IC_noise',v_IC_noise};
shared_parameters=cat(2,noise_parameters,fixed_biophysics);

% Network Specification
s=[];
s.pops(1)    =ACC_Ecell_specification(E_type,nE,heterogeneous_params,heterogeneity_degree,distribution);
s.pops(end).name='E';
s.pops(end).size=nE;
s.pops(end).parameters=cat(2,s.pops(end).parameters,{'baseline_rate',baseline_rate_E,'Iapp',Iapp_E,shared_parameters{:}});
if If_flag
  s.pops(end+1)=ACC_Icell_specification(If_type);
  s.pops(end).name='If';
  s.pops(end).size=nI;
  s.pops(end).parameters=cat(2,s.pops(end).parameters,{'baseline_rate',baseline_rate_I,'Iapp',Iapp_I,shared_parameters{:}});
end
if Is_flag
...
if 0
  ...
  Eleak=data.model.parameters.E_ileak_Eleak;
  data=CalcFR(data);
  num_spikes=cellfun(@length,data.E_v_spike_times);
  figure; plot(Eleak,num_spikes,'o-'); xlabel('Eleak'); ylabel('# spikes');
end

% characterize cells
if 0
  % check for heterogeneous model parameters
  params=[];
  if length(heterogeneous_params)==1 && isequal('Eleak',heterogeneous_params{1})
    param_label='Eleak'; params=data.model.parameters.E_ileak_Eleak;
  elseif length(heterogeneous_params)==1 && isequal('gh',heterogeneous_params{1})
    param_label='gh'; params=data.model.parameters.E_ih_gh;
  end
  % determine cell IPs
  pop='E';
  modl=ApplyModifications(data.model,{'(E,If,Is)','(baseline_rate,gNOISE,Iapp)',0}); % turn off inputs
  probedata=ProbeCellProperties(modl,'amplitudes',[-10 -7.5 -5 0:10:400],'tspan',[0 750],'compile_flag',1,'verbose_flag',1,'solver','rk2');
  PlotData(probedata(unique(round(linspace(1,length(probedata),10)))),'ylim',[-100 50])
  stats=CalcCellProperties(probedata,'plot_flag',0); 
  stats.(pop)
  % plot cell IPs
  IP_labels={'AHP_time2trough','RMP','Ih_abssag','min_ISI_median','ISI_step_median','AR23','ARif','AP_dur'};
  IP_lims={[0 40],[-90 -60],[0 2],[0 50],[0 50],[0 1.5],[0 1.5],[.5 2]};
  figure('position',[550 120 800 800]); cnt=0; num_IPs=length(IP_labels);
  for i=1:num_IPs
    subplot(num_IPs,2,cnt+1); ip=strrep(IP_labels{i},'_','\_');
    hist(stats.(pop).(IP_labels{i})); xlim(IP_lims{i}); xlabel(ip);
    subplot(num_IPs,2,cnt+2); 
    if ~isempty(params)
      plot(params,stats.(pop).(IP_labels{i}),'o-'); 
      xlabel(param_label); ylabel(ip); ylim(IP_lims{i});
    end
    fprintf('<%s> = %g\n',IP_labels{i},nanmedian(stats.(pop).(IP_labels{i})));
    cnt=cnt+2;
  end  
end
...
net_realization=1;
net_params_str = ''; % example: 'randcon_gII-0_gEI-1_gIE-.5_gEE-0'

% Prepare names according to parameter-based conventions:
% Prepare parameter-specific name for network
% Example: netID='ACd-super__Eclass1-20het-<dist>-<level>_Is-5hom__{fixed-net-params}__<realization>'
IhetID='hom';
if ~isempty(heterogeneous_params)
  EhetID=sprintf('het-%s-deg%g',distribution,heterogeneity_degree);
else
  EhetID='hom';
end
...
% USAGE:
% exp_subdir=[func2str(experiment_function) '__' get_strID(experiment_options,'options')];
    % example: 'ProbeTwoRhythms__dc-#_ac-#_f1-10-60_f2-10-60'
% study_dir=fullfile(net_study_dir,exp_subdir);
% study_dir=fullfile(net_study_dir,exp_subdir,get_strID(vary,'vary'));
